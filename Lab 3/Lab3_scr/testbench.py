...
def test_runCocotbTestsRegisterFile(pytestconfig):
    runCocotbTestsRegisterFile(pytestconfig)
def runCocotbTestsRegisterFile(pytestconfig):
    ...
    runr.test(
        ...
        waves=True,
        hdl_toplevel=toplevel_module, 
        test_module="testbench_regfile",
 # use tests from testbench_refile.py
        testcase=getattr(pytestconfig.option, "tests", None),
 # filter tests via the `--tests` command-line flag
    )
    pass
def test_runCocotbTestsProcessor(pytestconfig):
    runCocotbTestsProcessor(pytestconfig)
